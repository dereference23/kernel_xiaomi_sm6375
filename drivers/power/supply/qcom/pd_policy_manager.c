...
static const char *const bms_iio_chan_name[] = {
	[BMS_VOLTAGE_NOW] = "voltage_now", [BMS_CURRENT_NOW] = "current_now",
	[BMS_TEMP_NOW] = "temp",	   [BMS_CYCLE_COUNT] = "cycle_count",
...
static bool is_cp_chan_valid(struct usbpd_pm *chip, enum cp_iio_channels chan)
{
	...
	if (!chip->cp_iio[chan]) {
		chip->cp_iio[chan] =
			iio_channel_get(chip->dev, cp_iio_chan_name[chan]);
		if (IS_ERR(chip->cp_iio[chan])) {
				 ...
				 enum cp_iio_channels chan)
{
	...
	if (!chip->cp_sec_iio[chan]) {
		chip->cp_sec_iio[chan] =
			iio_channel_get(chip->dev, cp_sec_iio_chan_name[chan]);
		if (IS_ERR(chip->cp_sec_iio[chan])) {
...
static bool is_bms_chan_valid(struct usbpd_pm *chip, enum bms_iio_channels chan)
{
	int rc;
	...
	if (!chip->bms_iio[chan]) {
		chip->bms_iio[chan] =
			iio_channel_get(chip->dev, bms_iio_chan_name[chan]);
		if (IS_ERR(chip->bms_iio[chan])) {
			       ...
			       enum main_iio_channels chan)
{
	...
	if (!chip->main_iio[chan]) {
		chip->main_iio[chan] =
			iio_channel_get(chip->dev, main_iio_chan_name[chan]);
		if (IS_ERR(chip->main_iio[chan])) {
			rc = PTR_ERR(chip->bms_iio[chan]);
			if (rc == -EPROBE_DEFER)
			       ...
			       enum adpo_iio_channels chan)
{
	...
	if (!chip->apdo_iio[chan]) {
		chip->apdo_iio[chan] =
			iio_channel_get(chip->dev, apdo_iio_chan_name[chan]);
		if (IS_ERR(chip->apdo_iio[chan])) {
...
int usbpd_get_iio_channel(struct usbpd_pm *chg, enum cp_iio_type type,
			  int channel, int *val)
{
	...
	case CP_MASTER:
		if (!chg->isln8000flg) {
			...
			if (!is_cp_chan_valid(
				    chg, (channel + IIO_SECOND_CHANNEL_OFFSET)))
				return -ENODEV;
			iio_chan_list =
				chg->cp_iio[channel + IIO_SECOND_CHANNEL_OFFSET];
		}
		break;
	case CP_SLAVE:
		if (!chg->isln8000flg) {
			...
			if (!is_cp_psy_chan_valid(
				    chg, (channel + IIO_SECOND_CHANNEL_OFFSET)))
				return -ENODEV;
			iio_chan_list =
				chg->cp_sec_iio[channel +
...
int usbpd_set_iio_channel(struct usbpd_pm *chg, enum cp_iio_type type,
			  int channel, int val)
{
	struct iio_channel *iio_chan_list;
	int rc;
...
static bool usbpd_get_pps_status(struct usbpd_pm *pdpm)
{
	 ...
	 */
		...
		if (apdo_idx == -1) {
			...
			ret = usbpd_set_iio_channel(pdpm, APDO, APDO_MAX_VOLT,
						    pdpm->apdo_max_volt);
			if (ret < 0) {
				pr_err("set APDO_MAX_VOLT fail(%d)\n", ret);
				return ret;
			}
			ret = usbpd_set_iio_channel(pdpm, APDO, APDO_MAX_CURR,
						    pdpm->apdo_max_curr);
			if (ret < 0) {
...
static int usbpd_select_pdo(struct usbpd_pm *pdpm, u32 mV, u32 mA)
{
	...
	if (mA >= 6000 && pdpm->cp.vout_volt > 7000) {
		if (pdpm->cp.ibat_curr <= 4000) {
			mA = 3000;
		} else if (pdpm->cp.ibat_curr > 4000 &&
			   pdpm->cp.ibat_curr < 6000) {
			mA = 4000;
		}
				     ...
				     unsigned long event, void *data)
{
	...
	case TCP_NOTIFY_PD_STATE:
		...
		case PD_CONNECT_PE_READY_SNK_PD30:
			...
			schedule_delayed_work(&__pdpm->pd_work,
...
static int usbpd_pm_set_swchg_cap(struct usbpd_pm *pdpm, u32 aicr)
{
	...
	ret = usbpd_set_iio_channel(pdpm, MAIN, MAIN_INPUT_CURRENT_SETTLED,
				    aicr);
	if (ret < 0) {
...
*/
	if (!en) {
		usbpd_set_iio_channel(pdpm, MAIN, MAIN_INPUT_CURRENT_SETTLED,
				      2000);
		usbpd_set_iio_channel(pdpm, MAIN, MAIN_CHAGER_CURRENT, 100);
	} else {
		usbpd_set_iio_channel(pdpm, MAIN, MAIN_ENBALE_CHAGER_TERM,
				      true);
...
static void usbpd_pm_update_cp_status(struct usbpd_pm *pdpm)
{
	...
	ret = usbpd_get_iio_channel(pdpm, CP_MASTER, CHARGE_PUMP_SC_BUS_VOLTAGE,
				    &val1);
	if (!ret)
		pdpm->cp.vbus_volt = val1;

	ret = usbpd_get_iio_channel(pdpm, CP_MASTER, CHARGE_PUMP_SC_BUS_CURRENT,
				    &val1);
	if (!ret)
		pdpm->cp.ibus_curr_cp = val1;

	ret = usbpd_get_iio_channel(pdpm, CP_SLAVE, CHARGE_PUMP_SC_BUS_CURRENT,
				    &val1);
	if (!ret)
	...
	if (pdpm->isln8000flg) {
		...
		if (!ret) {
			...
			pdpm->cp.bat_therm_alarm =
				!!(val1 & BAT_THERM_ALARM_MASK);
			pdpm->cp.bus_therm_alarm =
				!!(val1 & BUS_THERM_ALARM_MASK);
			pdpm->cp.die_therm_alarm =
				!!(val1 & DIE_THERM_ALARM_MASK);
		}
	}
}
...
static void usbpd_pm_update_cp_status(struct usbpd_pm *pdpm)
{
	...
	ret = power_supply_get_property(
		pdpm->cp_psy, POWER_SUPPLY_PROP_SC_BATTERY_VOLTAGE, &val);
	if (!ret)
...
static int usbpd_pm_check_sw_enabled(struct usbpd_pm *pdpm)
{
	...
	ret = power_supply_get_property(
		pdpm->sw_psy, POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT_MAX,
		&val);
	if (!ret) {
...
static int usbpd_update_ibat_curr(struct usbpd_pm *pdpm)
{
	...
	ret = usbpd_get_iio_channel(pdpm, BMS, BMS_CURRENT_NOW, &val1);
	if (!ret)
		pdpm->cp.ibat_curr = -(int)(val1 / 1000);

	ret = usbpd_get_iio_channel(pdpm, BMS, BMS_VOLTAGE_NOW, &val1);
	if (!ret)
		pdpm->cp.vbat_volt = (int)(val1 / 1000);

	ret = usbpd_get_iio_channel(pdpm, BMS, BMS_CYCLE_COUNT, &val1);
	if (!ret)
...
static int battery_sw_jeita(struct usbpd_pm *pdpm)
{
	...
	if (bat_temp >= CHG_BAT_TEMP_MIN && bat_temp < CHG_BAT_TEMP_MAX) {
		...
		} else if (pdpm->cp.vbat_volt >= cycle_volt &&
			   pdpm->cp.vbat_volt < CHG_CUR_VOLT2) {
			pdpm->cp.set_ibat_cur = CHG_VOLT2_MAX_CUR;
			step_vbat = bat_step(pdpm, CHG_VOLT2_MAX_CUR);
		} else if (pdpm->cp.vbat_volt >= CHG_CUR_VOLT2 &&
			   pdpm->cp.vbat_volt < CHG_CUR_VOLT3) {
		...
		if (pdpm->cp.vbat_volt >= (pdpm->voltage_max - 10) &&
		    (pdpm->voltage_max > 4400))
			step_vbat = -1;
	...
	pr_err(">>>>temp %d pdpm->cp.ibus_curr_sw %d step_ibat %d, step_vbat %d, therm_curr %d, cycle %d\n",
	       bat_temp, pdpm->cp.ibus_curr_sw, step_ibat, step_vbat,
	       therm_curr, pdpm->cp.battery_cycle);
...
static int usbpd_update_apdo_data(struct usbpd_pm *pdpm)
{
	...
	ret = usbpd_get_iio_channel(pdpm, APDO, APDO_MAX_VOLT, &val1);
	if (!ret)
		pdpm->apdo_max_volt = val1;

	ret = usbpd_get_iio_channel(pdpm, APDO, APDO_MAX_CURR, &val1);
	if (!ret)
...
static int usbpd_pm_fc2_charge_algo(struct usbpd_pm *pdpm)
{
	...
	if (pdpm->cp.bat_therm_fault) { /* battery overheat, stop charge*/
		...
		pr_notice(
			"bat_ocp_fault:%d, bus_ocp_fault:%d, bat_ovp_fault:%d, \
                bus_ovp_fault:%d\n",
	...
	if ((pdpm->cp.vbus_volt > 7000) && (pdpm->cp.vbus_volt < 11000)) {
		for (i = 0; i < 6; i++) {
			...
			pr_err(">>>>>>>pdpm->cp.vbus_volt %d, vbus_old %d, ads_vol %d\n",
			       pdpm->cp.vbus_volt, pdpm->cp.vbus_volt_old,
			       ads_vol);
			if (ads_vol > 500 && ads_vol < 1200) {
				msleep(5);
				usbpd_get_iio_channel(
					pdpm, CP_MASTER,
					CHARGE_PUMP_SC_BUS_VOLTAGE,
					&pdpm->cp.vbus_volt);
				...
				if (i == 5 && pdpm->cp.batt_temp > 70)
					usbpd_pm_move_state(
						pdpm, PD_PM_STATE_FC2_EXIT);
				if (pdpm->cp.batt_temp > 50 &&
				    pdpm->cp.batt_temp <= 60)
					pdpm->cp.vbus_volt_old =
						pdpm->cp.vbus_volt;
			} else {
...
static int usbpd_pm_sm(struct usbpd_pm *pdpm)
{
	...
	if (pdpm->sw_psy) {
		ret = power_supply_get_property(
			pdpm->sw_psy, POWER_SUPPLY_PROP_CAPACITY, &val);
#ifdef WT_COMPILE_FACTORY_VERSION
		if (val.intval >= 79 || pps_change_to_stop() ||
		    (therm_curr < 2000))
			pdpm->pps_leave = true;
		else
			pdpm->pps_leave = false;
#else
		if (val.intval >= 98 || pps_change_to_stop() ||
		    (therm_curr < 2000))
			pdpm->pps_leave = true;
		else
			pdpm->pps_leave = false;
#endif
		ret = power_supply_get_property(
			pdpm->sw_psy, POWER_SUPPLY_PROP_VOLTAGE_MAX, &val);
	...
	if (!pdpm->pps_leave) {
		pr_err(">>>>>>>>>>>state phase :%d, vbus_vol %d, vbat_vol %d  vout %d, volatge_max %d pdpm->apdo_max_curr %d\n",
		       pdpm->state, pdpm->cp.vbus_volt, pdpm->cp.vbat_volt,
		       pdpm->cp.vout_volt, pdpm->voltage_max,
		       pdpm->apdo_max_curr);
		pr_err(">>>>>ibus_curr %d  ibus_curr_m %d, ibus_curr_s %d  ibat_curr %d\n",
		       pdpm->cp.ibus_curr + pdpm->cp_sec.ibus_curr,
		       pdpm->cp.ibus_curr_cp, pdpm->cp_sec.ibus_curr,
		       pdpm->cp.ibat_curr);
	...
	case PD_PM_STATE_ENTRY:
		...
		if (pdpm->cp.vbat_volt < pm_config.min_vbat_for_cp) {
			pr_notice("batt_volt-%d, waiting...\n",
				  pdpm->cp.vbat_volt);
		} else if (bat_temp >= CHG_BAT_TEMP_45 ||
	...
	case PD_PM_STATE_FC2_ENTRY_1:
		...
		if (pm_config.cp_sec_enable)
			pdpm->request_voltage =
				pdpm->cp.vbat_volt * 2 + BUS_VOLT_INIT_UP * 2;
		else
			pdpm->request_voltage =
	...
	case PD_PM_STATE_FC2_ENTRY_2:
		...
		if (pdpm->cp.vbus_error_low ||
		    pdpm->cp.vbus_volt < pdpm->cp.vbat_volt * 207 / 100) {
			tune_vbus_retry++;
		...
		if (tune_vbus_retry > 30) {
			pr_notice(
				"Failed to tune adapter volt into valid range, \
	...
	case PD_PM_STATE_FC2_ENTRY_3:
		pr_err("PD_PM_STATE_FC2_ENTRY3 pdpm->cp.charge_enabled %d, pm_config.cp_sec_enable %d, pdpm->cp.charge_enabled %d, pdpm->cp_sec.charge_enabled %d\n",
		       pdpm->cp.charge_enabled, pm_config.cp_sec_enable,
	...
	case PD_PM_STATE_FC2_TUNE:
		...
		if (ret == PM_ALGO_RET_THERM_FAULT) {
			...
			pr_notice(
				"Move to switch charging, will try to recover \
                    flash charging:%d\n",
				ret);
			recover = true;
			stop_sw = false;
...
static void usbpd_pm_workfunc(struct work_struct *work)
{
	...
	if ((pdpm->cp.set_ibat_cur - pdpm->cp.ibat_curr > 2000) &&
...
{
	struct usbpd_pm *pdpm =
		container_of(work, struct usbpd_pm, pd_work.work);
...
static void tcp_notify_workfunc(struct work_struct *work)
{
	...
	struct usbpd_pm *pdpm =
		container_of(work, struct usbpd_pm, tcp_work.work);
...
static void usb_psy_change_work(struct work_struct *work)
{
	...
	ret = power_supply_get_property(pdpm->usb_psy, POWER_SUPPLY_PROP_ONLINE,
					&propval);
...
static int usbpd_iio_init(struct usbpd_pm *pdpm)
{
	...
	pdpm->isln8000flg = false;
	pdpm->cp_iio = devm_kcalloc(pdpm->dev, ARRAY_SIZE(cp_iio_chan_name),
				    sizeof(*pdpm->cp_iio), GFP_KERNEL);
	if (!pdpm->cp_iio)
		return -ENOMEM;
	pdpm->cp_sec_iio =
		devm_kcalloc(pdpm->dev, ARRAY_SIZE(cp_sec_iio_chan_name),
			     sizeof(*pdpm->cp_sec_iio), GFP_KERNEL);
	if (!pdpm->cp_iio)
		return -ENOMEM;
	pdpm->bms_iio = devm_kcalloc(pdpm->dev, ARRAY_SIZE(bms_iio_chan_name),
				     sizeof(*pdpm->bms_iio), GFP_KERNEL);
	if (!pdpm->bms_iio)
		return -ENOMEM;
	pdpm->main_iio = devm_kcalloc(pdpm->dev, ARRAY_SIZE(main_iio_chan_name),
				      sizeof(*pdpm->main_iio), GFP_KERNEL);
	if (!pdpm->main_iio)
		return -ENOMEM;
	pdpm->apdo_iio = devm_kcalloc(pdpm->dev, ARRAY_SIZE(apdo_iio_chan_name),
				      sizeof(*pdpm->apdo_iio), GFP_KERNEL);
	if (!pdpm->apdo_iio)
		return -ENOMEM;
...
static int usbpd_pm_probe(struct platform_device *pdev)
{
	...
	if (pdev->dev.of_node) {
		pdpm = devm_kzalloc(&pdev->dev, sizeof(struct usbpd_pm),
				    GFP_KERNEL);
		if (!pdpm) {
